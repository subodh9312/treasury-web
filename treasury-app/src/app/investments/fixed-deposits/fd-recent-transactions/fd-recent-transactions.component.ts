...
import { AddInterestRateComponent } from '../../add-interest-rate/add-interest-rate.component';

@Component({
  selector: 'app-fd-recent-transactions',
  templateUrl: './fd-recent-transactions.component.html',
  styleUrls: ['./fd-recent-transactions.component.css']
})
export class FdRecentTransactionsComponent implements OnInit, OnChanges {
  ...
  columnDefs = [
    ...
    { colId: 8, headerName: 'Callable', field: 'callable' }
  ];
  ...
  onGridReady(params) {
    ...
    var datasource: IDatasource = {
      getRows: (params: IGetRowsParams) => {
        ...
        this.transactionService.getAllFixedDepositInvestment(page, this.gridOptions.paginationPageSize, true)
          .subscribe(data => {
  ...
  onSell() {
    this.sellFixedDeposit.emit(this.selectedRowNode.data);
  }
  ...
  onAddInterestRate(RowData) {
    const dialogRef = this.dialog.open(AddInterestRateComponent, {
      data: { RowData: RowData, InvNumber: RowData.fdrNumber },
      height: '600px',
      width: '1200px'
    });
  }
